...
class App(cevent.CEvent):
    ...
    def mainUI(self):
        ...
        #format surfaces
        try:
            self.temp_humi_surf = self.bigfont.render(str(self._ht_values[0]) +u"\u00B0C  " + str(self._ht_values[1]) + "%" , True , self.white)    #u"\u00B0C" = °C in unicode 
        except:
        ...
        self.weather_temp_surf = self.middlefont.render(str(int(self.weather[0]))+u"\u00B0C", True, self.white)
        self.weather_temp_min_surf = self.tinyfont.render(str(int(self.weather[1]))+u"\u00B0C", True, self.blue)
        self.weather_temp_max_surf = self.tinyfont.render(str(int(self.weather[2]))+u"\u00B0C", True, self.blue)
    ...
    def forecastUI(self):
        ...
        while fcc < 8:
            ...
            forecast_temp_min_surf = forecast_temp_min_surf + (self.tinyfont.render(str(int(self.weather[6][fcc]))+u"\u00B0C", True, self.light_grey) , )
            forecast_temp_max_surf = forecast_temp_max_surf + (self.tinyfont.render(str(int(self.weather[7][fcc]))+u"\u00B0C", True, self.light_grey) , )
